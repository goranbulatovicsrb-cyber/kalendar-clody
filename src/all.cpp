...
void MusicPlayer::setupUI() {
    ...
    m_playlistFrame->setStyleSheet(
        "QFrame { background: rgba(8,8,28,0.95); border-radius:8px; border:1px solid rgba(153,102,204,0.3); }");
    ...
    m_playlistWidget->setMinimumHeight(120);
    m_playlistWidget->setMaximumHeight(200);
    m_playlistWidget->setStyleSheet(
        "QListWidget { background:rgba(5,5,20,0.9); border:1px solid rgba(255,255,255,0.1); border-radius:6px; color:#e0e0ff; font-size:12px; }"
        "QListWidget::item { padding:5px 8px; border-radius:4px; color:#e0e0ff; }"
        "QListWidget::item:hover { background:rgba(153,102,204,0.2); color:#ffffff; }"
...
void MusicPlayer::playIndex(int index) {
    ...
    for (int i = 0; i < m_playlistWidget->count(); ++i) {
        ...
        if (i == index) {
            item->setForeground(QColor("#ff6b8a"));
            item->setText("▶  " + extractTitle(m_playlist[i]));
        } else {
            item->setForeground(QColor("#c8c8e8"));
            item->setText("     " + extractTitle(m_playlist[i]));
        }
        QFont f = item->font();
        f.setBold(i == index);
...
void MainWindow::setupUI() {
    ...
    playerFrame->setMinimumHeight(160);
    playerFrame->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Minimum);
