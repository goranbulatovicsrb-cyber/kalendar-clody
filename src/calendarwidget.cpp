...
void CalendarWidget::paintEvent(QPaintEvent *) {
    ...
    // Header separator — naglašena linija ispod naziva dana
    p.setPen(QPen(QColor(150,120,220,120), 1.5f));
    p.drawLine(0, (int)(m_headerH + m_dayH - 1), width(), (int)(m_headerH + m_dayH - 1));

    // Vertikalne linije stupaca (po cijeloj visini grida)
    {
        float gridTop    = m_headerH + m_dayH;
        float gridBottom = height();
        p.setPen(QPen(QColor(255,255,255,35), 0.8f));
        float cellW2 = (float)width() / 7.0f;
        for (int c = 1; c < 7; ++c) {
            float x = c * cellW2;
            p.drawLine(QPointF(x, gridTop), QPointF(x, gridBottom));
        }
        // Horizontalne linije redova
        for (int row = 1; row < m_gridRows; ++row) {
            float y = gridTop + row * cellH;
            p.drawLine(QPointF(0, y), QPointF(width(), y));
        }
        // Vanjski okvir grida
        p.setPen(QPen(QColor(150,120,220,60), 1.0f));
        p.drawRect(QRectF(0, gridTop, width()-1, gridBottom - gridTop - 1));
    }
                                  ...
                                  bool hasEvents) const
{
    ...
    // Grid lines — vidljive, ali blage
    p.setPen(QPen(QColor(255,255,255,55), 0.8f));
    // Vertikalna linija (desna ivica svake ćelije osim zadnje u redu)
    if (date.dayOfWeek() < 7) {
        p.drawLine(r.topRight(), r.bottomRight());
    }
    // Horizontalna linija (donja ivica)
    p.drawLine(r.bottomLeft(), r.bottomRight());

    // Malo jači ugaoni punkt za estetiku
    p.setPen(QPen(QColor(255,255,255,30), 1.0f));
